...
public final class ProcessRunner {
    ...
    public static CommandResult run(String[] argv, long timeoutMillis) {
        ...
        try {
            ...
            Thread reader = new Thread(() -> {
                try (InputStream input = stream) {
                    ...
                    while ((count = input.read(buffer)) != -1) {
                        synchronized (bytes) {
                            if (bytes.size() < LIMIT) bytes.write(buffer, 0, Math.min(count, LIMIT - bytes.size()));
                        }
                    }
                } catch (Exception ignored) { /* destroyed process closes its stream */ }
            }, "command-output");
            ...
            while (true) {
                try { code = process.exitValue(); break; }
                    ...
                    if (System.nanoTime() >= deadline) {
                        process.destroy();
                        return new CommandResult(-2, "Command timed out; outcome unknown. Restoration retained.");
                    }
                    Thread.sleep(20);
                }
            }
            reader.join(1000);
            synchronized (bytes) {
                return new CommandResult(code, new String(bytes.toByteArray(), StandardCharsets.UTF_8).trim());
            }
