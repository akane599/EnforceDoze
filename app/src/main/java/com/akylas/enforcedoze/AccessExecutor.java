...
public final class AccessExecutor {
    ...
    static final ExecutorService RPC = new java.util.concurrent.ThreadPoolExecutor(1, 1, 0,
    ...
    public CommandResult run(String mode, String command) {
        ...
        if (command.startsWith("@")) {
            if (!mode.equals("root")) return new CommandResult(-1, "This control requires Shizuku or root.");
            String helper = "CLASSPATH=" + CommandResult.quote(context.getApplicationInfo().sourceDir)
                    + " app_process /system/bin com.akylas.enforcedoze.PrivilegedCommand " + CommandResult.quote(command);
            ...
            } catch (Exception e) { return new CommandResult(-1, "Root helper did not return a valid result."); }
        }
