...
public final class Automation {
    ...
    public static String token(Context c) {
        ...
        if(token==null) { token=UUID.randomUUID().toString()+UUID.randomUUID(); p.edit().putString("automationToken",token).commit(); }
        return token;
    }
    ...
    public static void whitelist(BroadcastReceiver receiver, Context c,Intent i,boolean add) {
        ...
        AccessExecutor.SERIAL.execute(() -> {
            try {
                CommandResult r=new AccessExecutor(c).run("dumpsys deviceidle whitelist "+(add?"+":"-")+CommandResult.quote(pkg));
                new EvidenceStore(c).record("Automation exemption",pkg+": "+(r.ok()?"command accepted; check exemption list":r.output));
