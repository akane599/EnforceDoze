...
public final class RestorationJournal {
    ...
    public interface Evidence { void record(String event, String detail); }
    public static final class Entry {
        public final String key, mode, query, kind, original, target, apply, undo;
        public Entry(String key, String mode, String query, String kind, String original,
                     String target, String apply, String undo) {
            this.key = key; this.mode = mode; this.query = query; this.kind = kind;
            this.original = original; this.target = target; this.apply = apply; this.undo = undo;
        }
    }
    ...
    public RestorationJournal(Store store, Device device, Evidence evidence) {
        this.store = store; this.device = device; this.evidence = evidence;
    }
    public boolean apply(Entry entry) {
        ...
        if (!store.save(entries)) {
            evidence.record("Failed", entry.key + ": recovery record could not be saved; no command sent");
            return false;
        }
        try {
            if (device.write(entry, false) && entry.target.equals(device.read(entry))) {
                evidence.record("Applied and observed", entry.key + ": " + entry.original + " → " + entry.target);
                return true;
            }
        } catch (Exception e) { evidence.record("Failed", entry.key + ": " + e.getMessage()); }
    ...
    public boolean restoreMatching(Selection selected) {
        ...
        for (int i = entries.size() - 1; i >= 0; i--) {
            ...
            try {
                ...
                if (!done && before != null) done = device.write(entry, true) && entry.original.equals(device.read(entry));
                if (done) {
            ...
            } catch (Exception e) { evidence.record("Restore failed", entry.key + ": " + e.getMessage()); }
            restored = false;
            evidence.record("Restoration pending", entry.key + " → " + entry.original + ". Reconnect " + entry.mode + " and retry.");
        }
        return restored;
    }
}
