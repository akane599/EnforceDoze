...
public final class StateParser {
    ...
    public static String read(String kind, CommandResult result) {
        ...
        if (kind.startsWith("suspended:"))
            return match(
                    text, "User " + kind.substring(10) + ": [^\\r\\n]*?suspended=(true|false)");
        switch (kind) {
            case "deep":
                return match(text, "(?:^|\\s)mState=([A-Z_]+)(?:\\s|$)");
            case "forced":
                return match(text, "(?:^|\\s)mForceIdle=(true|false)(?:\\s|$)");
            case "sensor":
                return match(text, "^\\s*Mode\\s*:\\s*([^\\r\\n]+)");
            case "boolean":
                return text.equals("true") || text.equals("false") ? text : null;
            case "switch":
                return text.equals("0") || text.equals("1") ? text : null;
            case "setting":
