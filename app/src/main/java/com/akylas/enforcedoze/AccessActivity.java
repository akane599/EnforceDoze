...
public class AccessActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle bundle) {
        ...
        screen("Access & permissions", true);
        LinearLayout card =
                card(
                        "Privileged access",
                    ...
                    new com.google.android.material.dialog.MaterialAlertDialogBuilder(this)
                            ...
                            .setItems(
                                    new String[] {
                                        "Shizuku (recommended)",
                                        "Root",
                                        "ADB-granted app access (legacy)"
                                    },
                                    (d, index) -> {
                                        if (new RecoveryStore(this).pending()) {
                                            message(
                                                    "Restore first",
                                        ...
                                        PreferenceManager.getDefaultSharedPreferences(this)
                                                .edit()
                                                .putString("executionMode", values[index])
                                                .apply();
        ...
        button(
                        card,
                        "Authorize Shizuku",
                        () -> ShizukuHandler.getInstance(this).requestShizukuPermission())
                .setId(R.id.authorize_shizuku);
        button(
                card,
                "Verify access",
                () -> {
                                ...
                                String result =
                                        r.ok()
                                                ? "Command identity: uid "
                                                        + r.output
                                                        + ". Each device control is verified"
                                                        + " separately."
                                                : r.output;
        ...
        button(
                card,
                "Check or enable Android Doze",
                () -> {
                    new com.google.android.material.dialog.MaterialAlertDialogBuilder(this)
                            .setTitle("Enable Android Doze?")
                            .setMessage(
                                    "This enables Android’s own light and deep idle controllers if"
                                        + " disabled. It does not force a session or prove this OEM"
                                        + " supports every Doze behavior.")
        ...
        button(
                permissions,
                "Notification access for media protection",
                () -> safeStart(new Intent(Settings.ACTION_NOTIFICATION_LISTENER_SETTINGS)));
        button(
                permissions,
                "Allow precise schedule alarms",
                () -> {
        ...
        card(
                "Legacy ADB access",
                "ADB-granted DUMP and WRITE_SECURE_SETTINGS permissions can support older Android"
                    + " versions. On Android 14+, these permissions do not grant device_config or"
                    + " force-idle access. Use Shizuku or root for automatic Deep Doze. Never"
                    + " assume a permission grant means a feature succeeded.");
        button(
                body,
                "Copy legacy ADB setup commands",
                () ->
                        copy(
                                "ADB setup",
                                "adb shell pm grant "
    ...
    protected void onStart() {
        super.onStart();
        ShizukuHandler.getInstance(this).addListener(listener);
        refresh();
    }

    @Override
    protected void onStop() {
        ShizukuHandler.getInstance(this).removeListener(listener);
        super.onStop();
    }
}
