...
public final class EvidenceStore implements RestorationJournal.Evidence {
    ...
    @Override public void record(String event, String detail) {
        synchronized (LOCK) {
            try {
                JSONArray previous = new JSONArray(prefs.getString("events", "[]"));
                JSONArray next = new JSONArray();
                for (int i = Math.max(0, previous.length() - 199); i < previous.length(); i++) next.put(previous.get(i));
    ...
    public String text() {
        synchronized (LOCK) {
            StringBuilder text = new StringBuilder("Saved observations, not continuous monitoring.\nSensor access restrictions do not prove physical sensors are powered off.\n\n");
            try {
                JSONArray events = new JSONArray(prefs.getString("events", "[]"));
                if (events.length() == 0) text.append("No observations yet. Start monitoring, then turn the screen off and on.\n");
                for (int i = events.length() - 1; i >= 0; i--) {
                    JSONObject e = events.getJSONObject(i);
                    text.append(DateFormat.getDateTimeInstance().format(new Date(e.getLong("time"))))
                            .append(" • ").append(e.getString("event")).append('\n')
                            .append("Screen ").append(e.getBoolean("screenOn") ? "on" : "off")
                            .append(" • Android deep-idle API: ").append(e.getBoolean("idle"))
                            .append(" • uptime ").append(e.getLong("elapsed") / 1000).append("s\n")
                            .append(e.getString("detail")).append("\n\n");
                }
            } catch (Exception e) { text.append("Saved observations could not be read: ").append(e.getMessage()); }
