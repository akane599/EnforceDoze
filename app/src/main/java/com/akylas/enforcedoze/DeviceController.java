...
public final class DeviceController implements RestorationJournal.Device {
    ...
    public boolean setting(String key, String namespace, String name, String target) {
        ...
        return change(key, prefix + "get " + namespace + " " + name, "setting", target,
                ...
                original -> prefix + (original.equals("null") ? "delete " + namespace + " " + name
                        : "put " + namespace + " " + name + " " + CommandResult.quote(original)));
    }
    public boolean restrictSensors(String exemptPackage) {
        ...
        if (!"NORMAL".equals(before)) {
            evidence.record("Sensor restriction skipped", "Existing SensorService mode: " + before + "; not owned by EnforceDoze");
            return false;
        }
    ...
    public boolean forceIdle() {
        ...
        if (!"false".equals(original)) {
            evidence.record("Doze skipped", "Force-idle state is " + original + "; cannot take ownership");
            return false;
        }
    ...
    public boolean pending() { return store.pending(); }
    public List<RestorationJournal.Entry> entries() { return store.load(); }
    @Override public String read(RestorationJournal.Entry entry) {
        return StateParser.read(entry.kind, access.run(entry.mode, entry.query));
    }
    @Override public boolean write(RestorationJournal.Entry entry, boolean restore) throws Exception {
        CommandResult result = access.run(entry.mode, restore ? entry.undo : entry.apply);
        if (!result.ok()) {
            evidence.record(restore ? "Restore command failed" : "Command failed", entry.key + ": " + result.output);
            return false;
        }
        String expected = restore ? entry.original : entry.target;
        // Commands such as Wi-Fi return before the service changes state. Bounded verification only.
        for (int i = 0; i < 4; i++) {
