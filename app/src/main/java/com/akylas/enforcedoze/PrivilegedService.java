...
public class PrivilegedService extends IPrivilegedService.Stub {
    ...
    @Override
    public synchronized String run(String command) {
                            ...
                            new CommandResult(
                                    ...
                                    String.valueOf(
                                            api.getMethod(
                                                            ...
                                                            int.class,
                                                            int.class)
                                                    .invoke(service, subId, 0)));
                }
            } catch (Exception e) {
                result = new CommandResult(-1, "Device control unavailable: " + e);
            }
            ...
            try {
                IBinder binder =
                        (IBinder)
                                Class.forName("android.os.ServiceManager")
                                        .getMethod("getService", String.class)
                                        .invoke(null, "sensor_privacy");
                Class<?> api = Class.forName("android.hardware.ISensorPrivacyManager");
                Object service =
                        Class.forName("android.hardware.ISensorPrivacyManager$Stub")
                                .getMethod("asInterface", IBinder.class)
                                .invoke(null, binder);
                if (command.equals("@sensor-privacy true")
                        || command.equals("@sensor-privacy false")) {
                    api.getMethod("setSensorPrivacy", boolean.class)
                            .invoke(service, command.endsWith("true"));
                ...
                result =
                        new CommandResult(
                                0,
                                String.valueOf(
                                        api.getMethod("isSensorPrivacyEnabled").invoke(service)));
            } catch (Exception e) {
        ...
        try {
            ...
            return "{\"code\":-1,\"output\":\"Result encoding failed\"}";
        }
    }
